...
const initialSeqNum = seqNumInitialClient + 1 + 1 // seqNum after sending ACK of SYNACK

var cwnd uint32 = 1024
	...
	go func() {
		...
		for {
			if c.udpConn != nil {
				...
				case packet := <-c.ackPacket: // move window pointers
					...
					if packet.header.ackNum > w.base { // expected ack
						...
						if w.base < w.nextSeqNum || w.base < uint32(len(w.data))+initialSeqNum {
							...
							w.timeoutAck = nil
							debug("ENDING SENDING CURRENT DATA")
							c.resultWrite <- nil
							//break sending
						}
					} else { // duplicated ack
				...
				default:
					// define how much is missing to send
					limit := uint32(len(w.data)) + initialSeqNum - w.base
					if w.size < limit {
						limit = w.size
					}
					...
					if w.nextSeqNum < w.base+uint32(limit) { // if is possible to send
						...
						if end == uint32(len(w.data)) {
							// sent all data
							debug("SENT ALL WINDOW DATA")
						}
					}
				}
			} else {
				debug("ENDING WRITING TO CONN")
				break
			}
		}
	}()
...
func (c *ConnServer) demultiplexer(packet *Packet) {
	if packet.header.ack {
		...
		if packet.sourceAddr == c.remoteAddr {
			...
			// PACKET WITHOUT ACK WITHOUT KNOWN ID
			// IGNORE?
		}
	}
}
